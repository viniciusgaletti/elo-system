...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { processDescription, processName } = await req.json()
    ...
    const prompt = `Você é um consultor de IA especialista em processos de negócios.
Nome do Processo: ${processName}
Descrição: ${processDescription}

Com base nisso, liste as etapas sequenciais (entre 4 e 10 etapas). 
